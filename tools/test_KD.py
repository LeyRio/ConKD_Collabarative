...
def make_dataset_list(cfg, test_only=True):
    # if only for test then just build the dataset accroding to the cfg.data.test
    # then we can set it as list and return
    ...
    # if not just for test,then accroding to whether cfg.data.train.dataset has value
...
def main():
    ...
    if args.from_file:
        ...
        cfg.student_model.pretrained = None

        # build the dataloader
        dataset_list = make_dataset_list(cfg, args.testset_only)
        # build the model and load checkpoint
        model = build_classifier(cfg.student_model, train_cfg=None, test_cfg=cfg.test_cfg)
        checkpoint = load_checkpoint(model, args.checkpoint, map_location='cpu')

        # 构造保存数据的容器，如果之前已经保存过一部分就接着读入继续保存
        if osp.exists(args.out):
            savedata = mmcv.load(args.out)
        else:
            savedata = [dict() for _ in range(len(dataset_list))]
        for d, dataset in enumerate(dataset_list):
            # 如果只是为了测试，那么list中只有第一个dataset是用来test的，所以如果d>0 break就行
