...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        while True:
            if self.img_mode == 'twocrops':
                result_data = {}
                data = self.prepare_train_img(idx)
                data2 = self.prepare_train_img(idx)
                for k,v in data.items():
                    for m,n in data2.items():
                        if k == m:
                            result_data[k] = []
                            result_data[k].append(data[k])
                            result_data[k].append(data2[k])
            else:
                result_data = self.prepare_train_img(idx)
