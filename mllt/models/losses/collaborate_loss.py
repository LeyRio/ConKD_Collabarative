...
# change1:添加新的self参数(是否会在训练中更新？)
# change2:使用新的logit_reg_functions函数
# change3:是否能够成功import?
@LOSSES.register_module
class CollaborateLoss(nn.Module):
    ...
    def __init__(self,
        ...
        super(CollaborateLoss, self).__init__()
    ...
    def forward(self,
        ...
        # 首先在此处，将loss中用到的uik转化为uik * sigma p + ukp
        #weight = self.reweight_functions(label)
        weight = self.get_weight(cls_score,label)
        cls_score, weight = self.logit_compensation_functions(label.float(), cls_score, weight)
    ...
    def get_weight(self,cls_score,label):
        weight = label * torch.exp(1-self.rou) + (1-label) * torch.exp(self.rou)
        return weight

    # 重新设置了四个参数，重新给定初始值并根据正负样例进行处理
    def logit_compensation_functions(self,labels,logits,weight = None):
        logits = labels * (logits * self.pos_sigmas + self.pos_us) + (1-labels) * (logits * self.neg_sigmas + self.neg_us)
