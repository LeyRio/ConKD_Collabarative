...
class GCNTestClassifier(BaseClassifier):
    ...
    def __init__(self,
                 ...
                 con_loss = None):
        super(GCNTestClassifier, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(GCNTestClassifier, self).init_weights(pretrained)
    ...
    def forward_train(self,
        ...
        if flag == 1:
            ...
            x = self.backbone(img1)
            # 是否可以考虑和另一个分支的输出结果进行对比？
            #For main module:
            x_copy = self.backbone(img2)
            # 第一分支输出
            outs = self.neck1(x)
