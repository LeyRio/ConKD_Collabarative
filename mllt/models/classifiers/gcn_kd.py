...
class GCNKDClassifier(BaseClassifier):
    ...
    def __init__(self,
                 ...
                 savefeat=False,
                 T = 1.0):
        super(GCNKDClassifier, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        super(GCNKDClassifier, self).init_weights(pretrained)
    ...
    # TODO 设计蒸馏软标签损失函数
    # TODO 同时需要对教师模型最终输出进行调整，注意使用温度函数进行调节
    # TODO early stopping?
    ...
    def forward_train(self,
        ...
        if flag == 1:
            ...
            losses['gcn_losses'] = 0.05 * (self.multilabel_loss(outs2,gt_labels) + self.multilabel_loss(outs4,gt_labels))
            losses['soft_losses'] = F.kl_div(F.log_softmax(outs1/self.T,dim=1),
            ...
            #losses['gcn_losses'] = losses2['loss_cls']
            losses['gcn_losses'] = 0.05 * (self.multilabel_loss(outs_y2,gt_labels) + self.multilabel_loss(outs2_y2,gt_labels))
            losses['soft_losses'] = F.kl_div(F.log_softmax(outs_y1/self.T,dim=1),
                            F.softmax(soft/self.T,dim=1),reduction='batchmean')*self.T*self.T
